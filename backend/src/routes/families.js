...
router.post('/search', async (req, res, next) => {
  try {
    ...
    if (!phone_last4 && !qr_token) throw new AppError('Se requiere telefono o QR', 400);
    let sql, params;
    if (qr_token) {
      sql = `SELECT f.id,f.name,f.phone_primary,f.qr_token,f.checkin_count FROM families f WHERE f.qr_token=$1 AND f.is_active=TRUE AND f.organization_id=$2`;
      params = [qr_token, organization_id];
    } else {
      sql = `SELECT f.id,f.name,f.phone_primary,f.qr_token,f.checkin_count FROM families f WHERE RIGHT(f.phone_primary,4)=$1 AND f.is_active=TRUE AND f.organization_id=$2 ORDER BY f.checkin_count DESC LIMIT 5`;
    ...
    const kidRes = await query(`
      SELECT c.id,c.family_id,c.first_name,c.last_name,c.full_name,c.birthdate,c.age_years,
        ...
        cl.id AS suggested_classroom_id,
        cl.name AS suggested_classroom_name,
        cl.room_number AS suggested_room_number
      FROM children c
      LEFT JOIN classrooms cl ON cl.organization_id=c.organization_id AND cl.is_active=TRUE
        ...
        AND cl.sort_order = (
          SELECT MIN(cl2.sort_order) FROM classrooms cl2
          WHERE cl2.organization_id=c.organization_id AND cl2.is_active=TRUE
          AND (cl2.min_age IS NULL OR c.age_years >= cl2.min_age)
          AND (cl2.max_age IS NULL OR c.age_years <= cl2.max_age)
        )
      WHERE c.family_id=ANY($1) AND c.is_active=TRUE
      ORDER BY c.age_years DESC NULLS LAST
    `, [famIds]);
...
router.get('/:id/history', async (req, res, next) => {
  try {
    const result = await query(`
      SELECT ci.id,ci.security_code,ci.checked_in_at,ci.checked_out_at,
        c.full_name AS child_name,e.name AS event_name,cl.name AS classroom_name
      FROM checkins ci
...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!name || !phone_primary) throw new AppError('Nombre y telefono requeridos', 400);
    const result = await query(`
